...
try:
    import my_omnipose, edt, fastremap
    OMNI_INSTALLED = True
except:
...
def normalize99(Y,lower=0.01,upper=99.99,omni=False):
    ...
    if omni and OMNI_INSTALLED:
        X = my_omnipose.utils.normalize99(Y)
    else:
        X = Y.copy()
...
def random_rotate_and_resize(X, Y=None, scale_range=1., gamma_range=0.5, tyx=None, 
    ...
    if omni and OMNI_INSTALLED:
        ...
        return my_omnipose.core.random_rotate_and_resize(X, Y=Y, scale_range=scale_range, gamma_range=gamma_range,
...
def normalize_field(mu,omni=False):
    if omni and OMNI_INSTALLED:
        mu = my_omnipose.utils.normalize_field(mu) 
    else:
